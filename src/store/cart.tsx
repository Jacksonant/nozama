...
interface CartStore extends Cart {
  ...
  removeItem: (productId: number, selectedVariant?: { color?: ProductColor }) => void;
  updateQuantity: (productId: number, quantity: number, selectedVariant?: { color?: ProductColor }) => void;
  clearCart: () => void;
}

const calculateTotal = (items: Cart["items"]) =>
  items.reduce((sum, item) => sum + parseFloat(item.product.price || '0') * item.quantity, 0);
      ...
      (set, get) => ({
        ...
        addItem: (product, selectedColor, quantity = 1) => {
          ...
          if (existingItem) {
            ...
            const newItems = [...items, { 
              product, 
              quantity, 
              selectedVariant: selectedColor ? { color: selectedColor } : undefined 
            }];
        ...
        removeItem: (productId, selectedVariant) => {
          ...
          const removedItem = items.find(item => 
            ...
            item.selectedVariant?.color?.hex_value === selectedVariant?.color?.hex_value
          );
